...
class TodoModel(db.Model):
    ...
    def to_dict(self):
        return {"id": self.id, "title": self.title, "description": self.description, "status": self.status, "priority": self.priority, "category_id": self.category_id}
...
def create_todo():
    ...
    todo = TodoModel(title=data["title"], description=data.get("description", ""), status=data.get("status", "pending"), priority=data.get("priority", "medium"), category_id=data.get("category_id"))
...
def update_todo(todo_id):
    ...
    for field in ['title', 'description', 'status', 'priority', "category_id"]:
        if field in data:
            # todo[field] = data[field] # i guess since we are working with sqlachemy objects, we cannot do it this way
